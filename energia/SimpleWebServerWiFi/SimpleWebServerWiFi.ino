...
WiFiServer server(80);//port no of the http
...
void loop() {
  int i = 0;
  WiFiClient client = server.available();   // listen for incoming clients returns the boolenan values
      ...
      if (client.available()) {             // if there's bytes to read from the client,
        ...
        if (c == '\n') {                    // if the byte is a newline character
          ...
          if (strlen(buffer) == 0) {
            ...
            client.println("<html><head><title>Vruhshab's TIVA WiFi Web Server</title></head><body align=center>");
            client.println("<h1 align=center><font color=\"red\">Welcome to the Vrushabh's TIVA WiFi Web Server</font></h1>");
