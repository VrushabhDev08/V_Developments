...
class SmartIrrigationSystemGUI:
    ...
    def create_widgets(self):
        ...
        sensor_labels = ["Moisture Sensor", "Moisture Sensor 1", "Moisture Sensor 2", "Moisture Sensor 3",
                          "Moisture Sensor 4","NPK Sensor 1", "NPK Sensor 2"]
        ...
        # Valve Values Display
        # valve_label = tk.Label(self.root, text="Valves:", font=('Arial', 18, 'bold'))
        # valve_label.grid(row=4, column=3, columnspan=2, pady=10)
        ...
        #     valve_label.grid(row=i + 5, column=2, padx=10, pady=5, sticky="w")

        #     valve_entry = tk.Entry(self.root, textvariable=self.get_variable_by_index(i), state="readonly", font=('Arial', 16))
        #     valve_entry.grid(row=i + 5, column=3, padx=10, pady=5)
        # Update Button
        #len(valve_labels) need to add 
        update_button = tk.Button(self.root, text="Update Values", command=self.update_values, font=('Arial', 16))
        update_button.grid(row=len(sensor_labels)  + 10, column=1, columnspan=2, pady=10)
    
    def update_values(self):
        global selected_crop , selected_stage  
    ...
    def get_selected_values(self):
        selected_crop,selected_stage
        selected_crop = self.crop_var.get()
        selected_stage = self.stage_var.get()
        return selected_crop, selected_stage
...
if __name__ == "__main__":
    ...
    root.mainloop()
get_values = SmartIrrigationSystemGUI()
crop, stage = get_values.get_selected_values()